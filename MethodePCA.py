...
def read_txt_spectrum(path: str):
    """
    Format spectromètre TPOP:

    Colonne 1 : junk (toujours 1) -> ignorée
    Colonne 2 : Raman shift (cm^-1)
    Colonne 3 : Counts CCD (intensité)
    ...
    data = np.loadtxt(
        path,
        delimiter=",",
        usecols=(1, 2)   # ignore colonne 1 automatiquement
    )

    x = data[:, 0]
    y = data[:, 1]

    # sécurité ordre croissant
    if x[0] > x[-1]:
        x = x[::-1]
        y = y[::-1]
